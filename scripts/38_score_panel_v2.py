...
# Same logic as scripts/22_run_all.py, but the study panel is defined inline
# (no trait_panel.json) and restricted to the four studies below.
...
panel=[
    dict(trait="Ulcerative colitis (Liu 2023)",    expected="IMMUNE", study="GCST90446794"),
    dict(trait="Type 1 diabetes (Robertson 2021)", expected="IMMUNE", study="GCST90013445"),
    dict(trait="Body mass index (UKB-WGS 2025)",   expected="BRAIN",  study="GCST90474606"),
    dict(trait="Intelligence (Savage 2018)",        expected="BRAIN",  study="GCST006250"),
]
if SMOKE: panel=panel[:1]
...
for tr in panel:
    tag=tr['study']
    if os.path.exists(f"{SP}/T_{tag}_scores.npz"):
    ...
    n_capped=len(df)
    if SMOKE: df=df.head(2)
    print(f"[{tag}] fetched={n_fetched} after_coding_filter={n_after_filter} capped={n_capped} scoring={len(df)}",flush=True)
    if not SMOKE:
    ...
    elapsed=time.time()-t_study0
    n_failed=len(df)-len(rr)
    print(f"{tr['trait'][:32]:34s} {tag:<16s} n_scored={len(rr):<4d} n_failed={n_failed:<3d} {elapsed:.0f}s",flush=True)
    if SMOKE:
